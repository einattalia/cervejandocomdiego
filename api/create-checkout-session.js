...
module.exports=async function handler(req,res){
 ...
 const {STRIPE_SECRET_KEY,SUPABASE_URL,SUPABASE_SERVICE_ROLE_KEY,STRIPE_PAYMENT_METHODS='card'}=process.env;
 ...
 try{
  ...
  const session=await stripe.checkout.sessions.create({mode:'payment',payment_method_types:methods.length?methods:['card'],customer_email:customer.email,client_reference_id:order.id,metadata:{order_id:order.id,order_code:code},line_items:lines.map(l=>({quantity:l.quantity,price_data:{currency:'brl',unit_amount:Math.round(l.unit_price*100),product_data:{name:l.beer_name_snapshot}}})),success_url:`${origin}/pedido-confirmado?pedido=${encodeURIComponent(code)}&session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${origin}/pedido-confirmado?pedido=${encodeURIComponent(code)}&status=cancelled`});
