...
function cleanAddress(v){if(!v||typeof v!=='object')return null;return {postalCode:String(v.postalCode||'').slice(0,12),city:String(v.city||'').slice(0,100),street:String(v.street||'').slice(0,220),number:String(v.number||'').slice(0,20),district:String(v.district||'').slice(0,100),complement:String(v.complement||'').slice(0,120)};}
module.exports=async function handler(req,res){
 ...
 try{
  ...
  if(body.ageConfirmed!==true)return json(res,403,{error:'A venda é permitida somente para maiores de 18 anos. Confirme sua idade para continuar.'});
  ...
  if(delivery.type==='delivery'){const a=cleanAddress(delivery.address);if(!a?.postalCode||!a.city||!a.street||!a.number||!a.district)return json(res,400,{error:'Preencha CEP, cidade, endereço, número e bairro para entrega.'});}
  ...
  const stripe=new Stripe(STRIPE_SECRET_KEY);const origin=`${req.headers['x-forwarded-proto']||'https'}://${req.headers.host}`;const methods=STRIPE_PAYMENT_METHODS.split(',').map(s=>s.trim()).filter(Boolean);const lineItems=lines.map(l=>({quantity:l.quantity,price_data:{currency:'brl',unit_amount:Math.round(l.unit_price*100),product_data:{name:l.beer_name_snapshot}}}));
  if(deliveryFee>0)lineItems.push({quantity:1,price_data:{currency:'brl',unit_amount:Math.round(deliveryFee*100),product_data:{name:'Taxa de entrega'}}});
  const session=await stripe.checkout.sessions.create({mode:'payment',payment_method_types:methods.length?methods:['card'],customer_email:customer.email,client_reference_id:order.id,metadata:{order_id:order.id,order_code:code,age_verified:'true'},line_items:lineItems,success_url:`${origin}/pedido-confirmado?pedido=${encodeURIComponent(code)}&session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${origin}/pedido-confirmado?pedido=${encodeURIComponent(code)}&status=cancelled`});
  await supabase.from('orders').update({stripe_checkout_session_id:session.id}).eq('id',order.id);
  return json(res,200,{url:session.url,orderCode:code,total});
